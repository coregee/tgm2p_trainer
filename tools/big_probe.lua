...
_G.big_probe=emu.add_machine_frame_notifier(function()
    ...
    for p=0,1 do
        ...
        local step=(big==1 or big==3) and 2 or 1
        ...
        if not preview[p] and st.play_state==10 and st.next_piece&15>=2 then
            assert((st.next_piece&0x200~=0)==(big>0),'opening preview flag P'..(p+1))
            preview[p]=true
            manager.machine.screens[':screen']:snapshot(output..'/preview-p'..(p+1)..'.png')
        end
        if not started[p] and st.play_state==2 then
            assert((st.active_piece&0x200~=0)==(big>0),'first active flag P'..(p+1))
            assert(st.game_mode&0x40==0,'trainer forged BIG game mode')
            assert(st.active_x==((big==1 or big==3) and 5 or 4),'spawn alignment P'..(p+1)..': '..st.active_x)
        ...
        if started[p] and not done[p] then
            ...
            if age==136 then choices[p+1]=big==0 and 3 or 0; publish() end
            ...
            if age==154 then
                ...
                assert((st.active_piece&0x200~=0)==(big>0),'mid-piece geometry changed')
